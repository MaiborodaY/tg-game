import type { EnemyDefinition, Point, TowerStats, TowerType } from "./types.ts";
...
const LEVEL_DAMAGE_MULTIPLIER = [1, 1.55, 2.3] as const;
const LEVEL_RATE_MULTIPLIER = [1, 0.91, 0.8] as const;

export function getTowerStats(type: TowerType, level: 1 | 2 | 3): TowerStats {
...
export function getTowerTotalInvestment(type: TowerType, level: 1 | 2 | 3): number {
  const definition = TOWER_DEFINITIONS[type];
