...
const resizeRules = [
  ...
  { maxSide: 192, pattern: /^ui\/profile\/energy-pack\.png$/ },
...
const smallerAssetRules = [
  {
    ...
    maximumQuality: 46,
    pattern: /^arena\/layers\/arena-tier-\d+(?:-(?:variant|scene)-\d+)?-(?:ambient|ambient-particles|back|front|front-trees|ground|mid)(?:-\d+)?\.png$/,
  },
  {
  ...
  {
    maximumAlphaQuality: 86,
    maximumQuality: 74,
    pattern: /^ui\/profile\/energy-pack\.png$/,
  },
  {
...
function getResizeMaxSide(relativePath) {
  if (originalSizeRules.some((pattern) => pattern.test(relativePath))) {
    return undefined;
  }
