...
test("battle result panel exposes rewards and xp progress", () => {
  ...
  assert.equal(domUiSource.includes('export type BattleResultPresentationStage = "loot" | "reward";'), true);
  ...
  assert.equal(domUiSource.includes("element.dataset.lootRarity = rarity"), true);
  assert.equal(domUiSource.includes('createLootDropTextChip(getShopRarityLabel(rarity), "rarity")'), true);
  assert.equal(domUiSource.includes("DAMAGE_BLOCK_ICON_ASSET_URL"), true);
  assert.equal(domUiSource.includes("DAMAGE_HIT_ICON_ASSET_URL"), true);
  assert.equal(domUiSource.includes('mace: "MACE"'), true);
  assert.equal(domUiSource.includes("return WEAPON_CLASS_LABELS[item.weaponClass];"), true);
  assert.equal(domUiSource.includes('chip.setAttribute("aria-label", `${statLabel} ${stat}`);'), true);
  assert.equal(domUiSource.includes('icon.className = "battle-result__loot-card-stat-icon";'), true);
  assert.equal(domUiSource.includes("icon.src = statIconUrl;"), true);
  assert.equal(domUiSource.includes("chip.append(icon, value);"), true);
  assert.equal(domUiSource.includes("battle-result__loot-card-stat-value--better"), true);
  ...
  assert.equal(mainSource.includes("battleResultPresentationStage"), true);
  assert.equal(mainSource.includes('battleResultPresentationStage === "loot" ? "Continue" : battleResultReturnLabel'), true);
  assert.equal(mainSource.includes("continueBattleResultLootPresentation"), true);
  assert.equal(stylesSource.includes(".battle-result--loot"), true);
  assert.equal(stylesSource.includes(".battle-result--reward-after-loot"), true);
  assert.equal(stylesSource.includes(".result-banner.battle-result--loot"), true);
  assert.match(stylesSource, /\.battle-result--loot \.battle-result__frame\s*\{[\s\S]*background: none;[\s\S]*filter: none;/);
  assert.match(stylesSource, /\.battle-result--loot\.battle-result--animating \.battle-result__frame\s*\{[\s\S]*780ms/);
  assert.match(stylesSource, /\.battle-result--loot\.battle-result--animating \.battle-result__eyebrow\s*\{[\s\S]*battle-result-loot-plaque-in/);
  assert.match(stylesSource, /\.battle-result--loot\.battle-result--animating \.battle-result__button\s*\{[\s\S]*battle-result-loot-button-in/);
  assert.match(stylesSource, /@keyframes battle-result-loot-plaque-in\s*\{[\s\S]*transform: translateY\(-10px\) scale\(0\.92\);/);
  assert.match(stylesSource, /@keyframes battle-result-loot-button-in\s*\{[\s\S]*transform: translateY\(12px\) scale\(0\.97\);/);
  assert.match(stylesSource, /\.battle-result--reward-after-loot\.battle-result--animating \.battle-result__frame\s*\{[\s\S]*0ms/);
  assert.match(stylesSource, /\.battle-result--reward-after-loot\.battle-result--animating \.battle-result__button\s*\{[\s\S]*360ms/);
  assert.match(stylesSource, /\.battle-result--loot \.battle-result__loot\s*\{[\s\S]*border: 0;[\s\S]*background: transparent;[\s\S]*box-shadow: none;/);
...
test("arena menu nests settings and can leave an active battle", () => {
  assert.equal(html.includes('data-arena-menu'), true);
  ...
  assert.equal(stylesSource.includes(".arena-menu__panel"), true);
  assert.equal(stylesSource.includes(".arena-menu__item--danger"), true);
  assert.equal(stylesSource.includes(".battle-screen--finished .arena-menu"), true);
});
...
test("city hero profile exposes attributes combat stats and equipment", () => {
  ...
  assert.equal(mainSource.includes("const portraitMirrorParents = [cityHeroWidgetRefs.profilePortrait].filter("), true);
  assert.equal(mainSource.includes("onOpen: (layout) => cityScene?.setProfilePreview(layout),"), true);
  assert.equal(mainSource.includes("onClose: () => cityScene?.setProfilePreview(),"), true);
  assert.equal(mainSource.includes("cityScene?.setProfilePreview();"), true);
  ...
  assert.equal(stylesSource.includes(".city-menu--equipment-preview-open .city-menu__hero"), true);
...
test("city equipment inventory cards share the textured rarity standard", () => {
  ...
  assert.match(inventoryItemRule, /grid-template-columns: 46px minmax\(0, 1fr\);/);
  assert.equal(stylesSource.includes(".city-equipment-menu__item-icon-frame"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__item-info"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__item-diff--better"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__item-diff--worse"), true);
  assert.match(detailsRule, /border: 2px solid var\(--shop-rarity-light, rgba\(255, 220, 146, 0\.42\)\);/);
  assert.match(detailsRule, /linear-gradient\(180deg, rgba\(26, 9, 4, 0\.62\), rgba\(8, 2, 1, 0\.42\)\)/);
  assert.match(detailsRule, /backdrop-filter: blur\(1px\);/);
});
...
test("city equipment inventory keeps long item lists scrollable", () => {
  ...
  assert.equal(stylesSource.includes("grid-auto-rows: minmax(62px, auto);"), true);
  assert.equal(stylesSource.includes("grid-auto-rows: minmax(58px, auto);"), true);
});
...
test("city equipment inventory uses display names instead of raw item names", () => {
  ...
  assert.equal(cityHeroUiSource.includes('button.setAttribute("aria-label", `${displayName}, ${statLabel} ${statValue}`);'), true);
  assert.equal(cityHeroUiSource.includes('name.textContent = displayName.toUpperCase();'), true);
  assert.equal(cityHeroUiSource.includes('rarityChip.textContent = getShopRarityLabel(rarity).toUpperCase();'), true);
  assert.equal(cityHeroUiSource.includes('kindChip.textContent = getCityEquipmentProductTypeLabel(product);'), true);
  assert.equal(cityHeroUiSource.includes('diffChip.textContent = diff > 0 ? `+${diff}` : String(diff);'), true);
  assert.equal(cityHeroUiSource.includes('statChip.style.setProperty("--city-equipment-item-stat-icon"'), true);
});
...
test("city equipment inventory uses paper doll filters and details overlay", () => {
  ...
  assert.equal(cityHeroUiSource.includes('button.dataset.cityEquipmentAll = "true";'), true);
  assert.equal(cityHeroUiSource.includes('button.dataset.cityEquipmentFilter = "true";'), true);
  assert.equal(cityHeroUiSource.includes('label.textContent = "All";'), true);
  assert.equal(cityHeroUiSource.includes("return categoryIds.size <= 0 ? [...CITY_EQUIPMENT_CATEGORIES]"), true);
  assert.equal(cityHeroUiSource.includes("{ includeAll: true }"), true);
  assert.equal(cityHeroUiSource.includes("{ includeFilter: true }"), true);
  assert.equal(cityHeroUiSource.includes("const handleEquippedSlotClick = (event: MouseEvent) =>"), true);
  assert.equal(cityHeroUiSource.includes('target?.closest<HTMLButtonElement>("[data-city-equipment-slot-category]")'), true);
  assert.equal(cityHeroUiSource.includes("menu.stage?.addEventListener(\"click\", handleEquippedSlotClick);"), true);
  assert.equal(cityHeroUiSource.includes("menu.stage?.removeEventListener(\"click\", handleEquippedSlotClick);"), true);
  assert.equal(cityHeroUiSource.includes("slot.dataset.cityEquipmentSlotCategory = group.categoryId;"), true);
  assert.equal(cityHeroUiSource.includes("slot.dataset.cityEquipmentSlotCategory = categoryId;"), true);
  assert.equal(cityHeroUiSource.includes("slot.dataset.cityEquipmentSlotProductKey = getCityEquipmentItemIdsKey(itemIds);"), true);
  assert.equal(cityHeroUiSource.includes("activeCategoryIds = new Set([categoryId]);"), true);
  ...
  assert.equal(cityHeroUiSource.includes("options.onUnequip(product.itemIds);"), true);
  assert.equal(cityHeroUiSource.includes('cityMenu?.classList.toggle("city-menu--equipment-preview-open", nextOpen);'), true);
  assert.equal(cityHeroUiSource.includes("options.onOpen?.(getCityEquipmentProfilePreviewLayout(menu));"), true);
  assert.equal(cityHeroUiSource.includes("options.onClose?.();"), true);
  assert.equal(cityHeroUiSource.includes("const stageRect = menu.stage?.getBoundingClientRect();"), true);
  assert.equal(cityHeroUiSource.includes("syncCityEquipmentWeaponSlotsPosition(menu, stageRect);"), true);
  assert.equal(cityHeroUiSource.includes('host.style.bottom = "auto";'), true);
  assert.equal(cityHeroUiSource.includes('getCityEquipmentProfilePreviewSlotRect(menu, "gloves")'), true);
  assert.equal(cityHeroUiSource.includes('getCityEquipmentProfilePreviewSlotRect(menu, "boots")'), true);
  assert.equal(cityHeroUiSource.includes("const weaponSlotsRect = menu.weaponSlots?.getBoundingClientRect();"), true);
  assert.equal(cityHeroUiSource.includes("? weaponSlotsRect.top"), true);
  assert.equal(cityHeroUiSource.includes("visualBottomY"), true);
  assert.equal(cityHeroUiSource.includes("getCityEquipmentProfilePreviewSideFitRect(menu)"), true);
  assert.equal(cityHeroUiSource.includes("fitTopY: headSlotRect ? headSlotRect.bottom : stageRect.top"), true);
  assert.equal(cityHeroUiSource.includes("fitWidth: sideFitRect?.width ?? stageRect.width"), true);
  assert.equal(cityHeroUiSource.includes("return { left, width };"), true);
  assert.equal(cityHeroUiSource.includes('class="city-equipment-menu__title"'), false);
  ...
  assert.equal(cityHeroUiSource.includes('class="city-equipment-menu__weapon-slots"'), true);
  assert.equal(cityHeroUiSource.includes('class="city-equipment-menu__details" hidden'), true);
  assert.equal(cityHeroUiSource.includes('class="city-equipment-menu__details-chips"'), true);
  assert.equal(cityHeroUiSource.includes('class="city-equipment-menu__details-equipped" hidden'), true);
  assert.equal(cityHeroUiSource.includes('class="city-equipment-menu__details-set" hidden'), true);
  assert.equal(cityHeroUiSource.includes('class="city-equipment-menu__details-equip"'), true);
  assert.equal(cityHeroUiSource.includes('menu.detailsEquipButton.textContent = isEquipped ? "UNEQUIP" : "EQUIP";'), true);
  assert.equal(cityHeroUiSource.includes("getEquippedShopProductDisplayStat(hero, product.itemIds, statKind)"), true);
  assert.equal(cityHeroUiSource.includes('const statText = isEquipped || currentStat === statValue ? `${statValue}` : `${currentStat} > ${statValue}`;'), true);
  assert.equal(cityHeroUiSource.includes('menu.detailsStat.setAttribute("aria-label", `${statLabel} ${statText}`);'), true);
  assert.equal(cityHeroUiSource.includes("DAMAGE_HIT_ICON_ASSET_URL"), true);
  assert.equal(cityHeroUiSource.includes("getHeroEquipmentSetBonusSummary(firstItem, previewEquipment)"), true);
  assert.equal(cityHeroUiSource.includes("renderCityEquipmentWeaponSlots(menu, hero);"), true);
  assert.equal(cityHeroUiSource.includes('const CITY_EQUIPMENT_WEAPON_SLOT_GROUPS'), true);
  assert.equal(cityHeroUiSource.includes('{ id: "bow", label: "Bow", modifier: "weapon-bow", area: "bow", slot: "weaponBow" }'), true);
  assert.equal(cityHeroUiSource.includes('{ id: "melee", label: "Melee weapon", modifier: "weapon-main", area: "melee", slot: "weaponMain" }'), true);
  assert.equal(cityHeroUiSource.includes('{ id: "shuriken", label: "Shuriken", modifier: "weapon-shuriken", area: "shuriken" }'), true);
  assert.equal(cityHeroUiSource.includes('"armory-shop__equipped-slot"'), true);
  assert.equal(cityHeroUiSource.includes('"weapon-shop__equipped-slot"'), true);
  assert.equal(cityHeroUiSource.includes('count.className = "weapon-shop__equipped-count";'), true);
  assert.equal(cityHeroUiSource.includes("getHeroShurikenItemId(hero)"), true);
  assert.equal(stylesSource.includes("grid-template-areas:"), true);
  assert.equal(stylesSource.includes('"body hero shield"'), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__weapon-slots"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__weapon-slot"), true);
  assert.equal(stylesSource.includes("--city-equipment-edge-inset: 4px;"), true);
  assert.equal(stylesSource.includes("--city-equipment-edge-inset: 3px;"), true);
  assert.equal(stylesSource.includes("--city-equipment-slot-top-inset: 0px;"), true);
  assert.equal(stylesSource.includes("--city-equipment-slot-bottom-inset: 2px;"), true);
  assert.equal(stylesSource.includes("--city-equipment-weapon-slot-size: clamp(52px, 14vw, 64px);"), true);
  assert.equal(stylesSource.includes("--city-equipment-weapon-slot-size: clamp(48px, 13vw, 58px);"), true);
  assert.equal(stylesSource.includes("--armory-equipped-slot-size: var(--city-equipment-weapon-slot-size);"), true);
  assert.equal(stylesSource.includes("--city-equipment-slot-size: clamp(58px, 15.5vw, 72px);"), true);
  assert.equal(stylesSource.includes("--city-equipment-slot-size: clamp(52px, 14.5vw, 64px);"), true);
  assert.equal(stylesSource.includes("calc(var(--city-equipment-weapon-slot-size) + var(--city-equipment-slot-bottom-inset))"), true);
  assert.equal(stylesSource.includes("width: min(calc(100% - var(--city-equipment-edge-inset) - var(--city-equipment-edge-inset)), 286px);"), true);
  assert.equal(stylesSource.includes("width: min(calc(100% - var(--city-equipment-edge-inset) - var(--city-equipment-edge-inset)), 266px);"), true);
  assert.equal(stylesSource.includes("grid-template-columns: var(--city-equipment-slot-size) minmax(0, 1fr) var(--city-equipment-slot-size);"), true);
  assert.equal(stylesSource.includes("grid-template-rows: repeat(5, var(--city-equipment-slot-size));"), true);
  assert.equal(stylesSource.includes("align-content: start;"), true);
  assert.equal(stylesSource.includes("gap: 2px;"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__slot--head"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__slot--body,\n.city-equipment-menu__slot--shield"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__hero-preview {\n  display: none;"), true);
  assert.equal(stylesSource.includes("grid-template-columns: repeat(3, var(--armory-equipped-slot-size));"), true);
  assert.equal(stylesSource.includes("backdrop-filter: none;"), true);
  assert.equal(stylesSource.includes(".city-menu--equipment-preview-open .city-equipment-menu__panel"), true);
  assert.equal(stylesSource.includes("animation: none;"), true);
  assert.equal(stylesSource.includes(".city-menu--equipment-preview-open .city-equipment-menu__equipped-slots"), true);
  assert.equal(stylesSource.includes(".city-menu--equipment-preview-open .city-equipment-menu__weapon-slots"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__details[hidden]"), true);
  assert.equal(stylesSource.includes("background: var(--city-equipment-stat-icon) center / contain no-repeat;"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__details-set-bonus--inactive"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__details-equipped"), true);
  ...
  assert.equal(stylesSource.includes("grid-template-rows: repeat(2, 48px);"), true);
  assert.equal(stylesSource.includes("grid-template-rows: repeat(2, 44px);"), true);
  assert.equal(stylesSource.includes("grid-auto-rows: 48px;"), true);
  assert.equal(stylesSource.includes("grid-auto-rows: 44px;"), true);
  assert.equal(stylesSource.includes("grid-template-rows: clamp(344px, 48vh, 372px) minmax(0, 1fr);\n  gap: 0;"), true);
  assert.equal(stylesSource.includes("grid-template-rows: clamp(330px, 47vh, 348px) minmax(0, 1fr);\n    gap: 0;"), true);
  assert.equal(stylesSource.includes("width: min(404px, 100%);"), true);
  assert.equal(stylesSource.includes("border: 0;"), true);
  assert.equal(stylesSource.includes("background: transparent;"), true);
  assert.equal(stylesSource.includes("box-shadow: none;"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__panel::before {\n  display: none;"), true);
  assert.equal(stylesSource.includes("grid-template-columns: repeat(7, minmax(0, 1fr));"), true);
  assert.equal(stylesSource.includes("grid-template-columns: repeat(10, minmax(0, 1fr));"), true);
  assert.equal(stylesSource.includes("border-bottom-width: 0;"), true);
  assert.equal(stylesSource.includes("border-top-width: 0;"), true);
  assert.equal(stylesSource.includes("margin-top: -2px;"), true);
  assert.equal(stylesSource.includes("padding: 0 3px 5px;"), true);
  assert.equal(stylesSource.includes("width: 94%;"), true);
  assert.equal(stylesSource.includes("height: 94%;"), true);
  assert.equal(stylesSource.includes("width: 68%;"), true);
  assert.equal(stylesSource.includes("height: 68%;"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__category--filter"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__category-icon--filter::before"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__slot:focus-visible"), true);
  assert.equal(stylesSource.includes(".city-equipment-menu__weapon-slot:focus-visible"), true);
  assert.equal((stylesSource.match(/pointer-events: auto;/g) ?? []).length >= 2, true);
});

test("city equipment inventory exposes generated weapon categories", () => {
  const swordCategoryIndex = cityHeroUiSource.indexOf('id: "swords", label: "Swords", side: "weapon"');
  const axeCategoryIndex = cityHeroUiSource.indexOf('id: "axes", label: "Axes", side: "weapon"');
  const maceCategoryIndex = cityHeroUiSource.indexOf('id: "maces", label: "Maces", side: "weapon"');
  const spearCategoryIndex = cityHeroUiSource.indexOf('id: "spears", label: "Spears", side: "weapon"');
  const bowCategoryIndex = cityHeroUiSource.indexOf('id: "bows", label: "Bows", side: "weapon"');
  const shurikenCategoryIndex = cityHeroUiSource.indexOf('id: "shurikens", label: "Shurikens", side: "weapon"');
  ...
  assert.equal(swordCategoryIndex >= 0, true);
  assert.equal(swordCategoryIndex < axeCategoryIndex, true);
  assert.equal(axeCategoryIndex < maceCategoryIndex, true);
  assert.equal(maceCategoryIndex < spearCategoryIndex, true);
  assert.equal(spearCategoryIndex < bowCategoryIndex, true);
  assert.equal(bowCategoryIndex < shurikenCategoryIndex, true);
