...
test("magic shop uses one selected scroll preview and a shared scroll cap counter", () => {
  ...
  assert.match(stylesSource, /@media \(max-width: 460px\), \(max-height: 760px\)\s*\{[\s\S]*\.magic-shop\.armory-shop--city-mode\s*\{[\s\S]*--magic-shop-title-actual-top: calc\(var\(--shop-frame-top-height\) \+ var\(--magic-shop-title-top\)\);/);
  ...
  assert.match(stylesSource, /body\.debug-mode-ui \.magic-shop\.armory-shop--city-mode\[data-ui-layout-viewport="compact"\]::after\s*\{[\s\S]*left: calc\(var\(--shop-frame-side-width\) \+ 50px\);/);
  ...
  assert.equal(uiLayoutTuningSource.includes('createControl("previewPriceWidth", "Preview price W"'), true);
  ...
  assert.equal(uiLayoutTuningSource.includes('"magicShop.buttons.previewPriceWidth.compact"'), true);
