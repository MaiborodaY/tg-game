...
test('legacy training migration preserves receipts without granting lancer historical credit', () => {
  ...
  assert.deepEqual(migrated.received, { swordsman: 12, archer: 3, healer: 0, lancer: 12, pantherRider: 0, elfArcher: 0 });
  assert.deepEqual(migrated.legacyTrainingCredit, { swordsman: 38, archer: 2, healer: 0, lancer: 0, pantherRider: 0, elfArcher: 0 });
...
test('unit catalogue keeps shared frozen definitions and optional healing fields', () => {
  assert.deepEqual(UNIT_TYPES.map(unit => unit.id), ['swordsman', 'archer', 'healer', 'lancer', 'pantherRider', 'elfArcher']);
  ...
  for (const unit of UNIT_TYPES) {
    ...
    assert.equal(Object.hasOwn(unit, 'heal'), unit.id === 'healer');
  }
});
