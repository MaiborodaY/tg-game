...
test('legacy training migration preserves receipts without granting lancer historical credit', () => {
  ...
  assert.deepEqual(migrated.received, { swordsman: 12, archer: 3, healer: 0, lancer: 12, pantherRider: 0, elfArcher: 0, elfHealer: 0 });
  assert.deepEqual(migrated.legacyTrainingCredit, { swordsman: 38, archer: 2, healer: 0, lancer: 0, pantherRider: 0, elfArcher: 0, elfHealer: 0 });
...
test('unit catalogue keeps shared frozen definitions and optional healing fields', () => {
  assert.deepEqual(UNIT_TYPES.map(unit => unit.id), ['swordsman', 'archer', 'healer', 'lancer', 'pantherRider', 'elfArcher', 'elfHealer']);
