...
const recruitmentAt = (swordsman, lancer = 0) => createRecruitment({ version: 2, received: { swordsman, lancer } });
...
test('all barracks actions reject inconsistent runtime state before changing it or charging gold', () => {
  ...
  const invalidStates = [null, {}, { ...valid, level: 4 }, { ...valid, firstLancerPending: true },
...
test('Barracks III timer survives reload and offline completion without granting another Lancer', () => {
  ...
  for (const pending of [false, true]) {
    ...
    const complete = getBarracksUpgrade(reloaded, recruitment, START + THIRD_DURATION);
    assert.equal(complete.status, 'complete');
    assert.equal(complete.targetLevel, null);
    assert.equal(complete.canStart, false);
    assert.equal(complete.lancerUnlocked, true);
    for (const field of ['cost', 'durationMs', 'remainingMs', 'speedUpCost', 'speedUpMaxCost']) assert.equal(complete[field], 0);
    assert.equal(startBarracksUpgrade(reloaded, recruitment, 5000, START + THIRD_DURATION).reason, 'max-level');
    assert.equal(speedUpBarracks(reloaded, 5000, START + THIRD_DURATION).reason, 'max-level');
...
test('Barracks III skip charges the current proportion of 300 gold including a stale displayed price', () => {
  ...
  for (const [elapsed, expectedCost] of [[0, 300], [HOUR, 200], [THIRD_DURATION / 2, 150], [THIRD_DURATION - 36_000, 1], [THIRD_DURATION - 1, 1]]) {
    ...
    assert.equal(barracks.firstLancerPending, false);
    assert.equal(speedUpBarracks(barracks, 300, START + elapsed).reason, 'max-level');
  }
