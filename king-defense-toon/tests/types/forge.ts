import { createForge, restoreForge, forgeUpgradeCost, getForgedUnitStats, upgradeForge } from '../../forge.ts';
...
export function verifyForgeContracts(saved: unknown): void {
  ...
  const upgrade: ForgeUpgrade = 'attack';
  const restored: { forge: ForgeState; refund: number } = restoreForge(saved);
  ...
  // @ts-expect-error Only the three shared upgrades may be purchased.
  upgradeForge(forge, 'hero', 50);
  // @ts-expect-error Removed ranged tracks cannot be purchased.
  upgradeForge(forge, 'rangedAttack', 50);
  // @ts-expect-error Normalized Forge state has no hidden ranged track.
  forge.rangedAttackSpeed = 10;
  ...
  // @ts-expect-error Normalized Forge state has all three tracks.
  const incomplete: ForgeState = { health: 1 };
  void [price, stats, result, incomplete, restored];
}
