...
const packUrl = new URL('../../art/brotd-infinity/allies/elves/panther-rider/glaive-v2/panther-glaive-rider-512.frames.json', import.meta.url);
const pack = JSON.parse(await readFile(packUrl, 'utf8'));
const projectile = JSON.parse(await readFile(new URL('moon-glaive-128.frames.json', packUrl), 'utf8'));
...
test('rider preserves the supplied irregular crops and foot anchors without inventing frames', async () => {
  ...
  assert.deepEqual(await readFile(new URL(PANTHER_RIDER_ASSETS[1].sheet)), await readFile(new URL(pack.image, packUrl)));
  ...
  for (const { index, rect, footAnchor } of pack.frames) {
    ...
    assert.equal(geometry.centers[index] * 128 + index % 4 * 128, rect.x + footAnchor.x);
    assert.equal(geometry.baselines[index] * 128 + Math.floor(index / 4) * 128, rect.y + footAnchor.y);
    assert.ok(rect.x >= 0 && rect.y >= 0 && rect.x + rect.width <= 512 && rect.y + rect.height <= 512);
  }
  assert.equal((await readFile(new URL('../panther-rider-art.ts', import.meta.url), 'utf8')).replaceAll('\r\n', '\n'), pantherRiderArtSource(pack, projectile));
});

test('rider throw release follows projectile launch, west mirrors the same poses, and death uses the normal fade', () => {
  ...
  for (const [facingX, facingY, start] of [[1, 0, 8], [-1, 0, 8], [0, -1, 8], [0, 1, 12]]) {
    const actor = { action: 'shoot', facingX, facingY, actionDuration: 1, impactFraction: .42 };
...
test('rider palettes preserve mount, skin, weapon, silhouettes and every alpha pixel', async () => {
  ...
  const frameByPixel = new Uint8Array(512 * 512);
  for (const frame of pack.frames) for (let y = frame.rect.y; y < frame.rect.y + frame.rect.height; y++) {
    frameByPixel.fill(frame.index, y * 512 + frame.rect.x, y * 512 + frame.rect.x + frame.rect.width);
  }
  for (const rank of [2, 3, 4, 5]) {
    ...
    for (let index = 0; index < source.data.length; index += 4) {
      ...
      const pixel = index / 4, x = pixel % 512, y = Math.floor(pixel / 512);
      ...
      if (frame.action === 'throw-down') assert.ok(y <= frame.rect.y + frame.footAnchor.y - 48, 'frontal mount and green eyes remain unchanged');
      changed++;
    }
...
test('rider body fits its two-cell footprint at the enlarged 54.05px reference size', async () => {
  ...
  const frame = pack.frames[0], scale = 54.05 / (PANTHER_RIDER_GEOMETRY.bodyHeight * 128);
  ...
  assert.ok((ground - top) * scale < 58.05, 'HP and level stay above the head and glaive');
