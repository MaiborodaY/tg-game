...
try {
  ...
  const results = await page.evaluate(async () => {
    ...
    window.renderHealer = (action, actionTime = 0, facingX = 1, facingY = 0, pulse = null, wave = 101) => {
      ...
      draws = [];
      scene.render({ battle, time: actionTime / .65 });
    ...
    for (const wave of [101, 151]) {
      for (const action of ['idle', 'walk']) for (const frame of [0, 1, 2, 3]) {
        rows.push(window.renderHealer(action, frame / (action === 'idle' ? 4 : 6) + .001, 1, 0, null, wave));
      }
      for (const [x, y] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
        for (const time of [0, .225, .45, .725]) rows.push(window.renderHealer('heal', time, x, y, null, wave));
      }
      for (const pulse of [0, .25, .5, .75]) rows.push(window.renderHealer('heal', .45, 1, 0, pulse, wave));
      rows.push(window.renderHealer('attack', .5, 1, 0, null, wave));
      rows.push(window.renderHealer('dead', 0, 1, 0, null, wave));
    }
    return rows;
  });
