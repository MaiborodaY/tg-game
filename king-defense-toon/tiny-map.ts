import type { LandBounds } from './field.ts';

export interface MapDrawBounds { top?: number }
export interface BattlefieldMap {
  width: number;
  height: number;
  backgroundColor: string;
  draw(context: CanvasRenderingContext2D, time?: number, bounds?: MapDrawBounds): void;
}

import { FIELD, BATTLE_VIEW, ROYAL_PENINSULA, ROYAL_NECK } from './field.ts';
...
function makeLayer() {
  ...
  // These renderers require Canvas 2D; retain the existing failure if it is unavailable.
...
function onRoad(col: number, row: number): boolean {
  if (row < -1 || row > 17) return false;
...
function drawTerrain(context: CanvasRenderingContext2D, ground: HTMLImageElement, hasTile: (col: number, row: number) => boolean, sourceX = 0): void {
...
function decoration(context: CanvasRenderingContext2D, image: HTMLImageElement, x: number, y: number, scale = 0.5): void {
...
function grassPatch(context: CanvasRenderingContext2D, ground: HTMLImageElement, bounds: LandBounds): void {
  const edge = 12;
...
export async function createTinyMap(): Promise<BattlefieldMap> {
  // Every ASSETS key is preserved by the parallel loader before the record is used.
