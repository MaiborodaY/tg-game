...
function encounterHealth(number) {
  return ROUND_HEALTH[Math.floor((number - 11) / 10)][(number - 1) % 10];
}

function meleeDamage(number) {
...
export function openingContinuationSpawns(number) {
  ...
  const guards = allocateHealth(spawns.filter(spawn => spawn.type !== bossType), health - bossHealth);
  let guardIndex = 0;
