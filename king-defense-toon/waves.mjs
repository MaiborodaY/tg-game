...
export function getEnemyCombatType(type) {
  return ENEMY_TYPES[type]?.combatType ?? type;
}

function defineWave(number, name, description, spawns, { stats = {}, bossOnly = false } = {}) {
  ...
  const resolvedSpawns = spawns.map(spawn => Object.freeze({ y: 92, ...stats[spawn.type], ...spawn }));
  ...
  return Object.freeze({
    ...
    hasBoss: bossType !== null, bossType, isFinalBossWave: Boolean(ENEMY_TYPES[bossType]?.isFinalBoss),
...
function squad(at, types, offset = 0) {
...
const REINFORCEMENT_TYPES = Object.freeze(['goblin', 'goblinArcher', 'goblin', 'boar']);
...
function campaignPressure(roundNumber, waveInRound) {
  const step = roundNumber === 1 ? 0 : (roundNumber - 2) * 9 + waveInRound;
  return { step, hp: 1 + .01 * step, damage: 1 + .008 * step };
}

function buildLaterForestWave(template, roundNumber) {
...
function buildForestWave(template, roundNumber) {
...
const GRAVEYARD_ENEMIES = Object.freeze({
...
function openingWave(wave) {
  ...
  const spawns = (number === 4 ? [
    ...
    return { ...spawn, hp, damage, reward: spawn.reward ?? ENEMY_TYPES[spawn.type].reward };
  });
...
export function getRoundWaves(levelNumber, roundNumber) {
  return getLevelWaves(levelNumber).filter(wave => wave.roundNumber === Number(roundNumber));
}

export function getWaveDefinition(number = 1) {
