...
export const HERO_BRANCHES = Object.freeze([
...
].map(talent => Object.freeze(talent)));

const TALENTS_BY_ID = Object.freeze(Object.fromEntries(HERO_TALENTS.map(talent => [talent.id, talent])));
const plainObject = value => value !== null && typeof value === 'object' && !Array.isArray(value);
const safeCount = value => Number.isSafeInteger(value) && value >= 0 ? value : 0;
export const heroXpForLevel = level => 50 * (Math.max(1, Math.min(HERO_MAX_LEVEL, Math.floor(Number.isFinite(level) ? level : 1))) - 1) ** 2;
const MAX_XP = heroXpForLevel(HERO_MAX_LEVEL);
const levelAt = xp => Math.min(HERO_MAX_LEVEL, 1 + Math.floor(Math.sqrt(xp / 50)));
const spentInBranch = (talents, branch) => HERO_TALENTS.reduce((sum, talent) => sum + (talent.branch === branch ? talents[talent.id] ?? 0 : 0), 0);

export function createHero(saved) {
  const source = plainObject(saved) ? saved : {};
  const xp = Math.min(MAX_XP, safeCount(source.xp));
  const level = levelAt(xp);
  const talents = Object.fromEntries(HERO_TALENTS.map(talent => [talent.id, 0]));
...
export function getHeroProgress(hero) {
...
export function getHeroStats(hero) {
  ...
  const scaled = base => Math.round(base * scale * 1e6) / 1e6;
  return {
...
export function getHeroTalentStatus(hero, id) {
...
export function spendHeroTalent(hero, id) {
...
export function resetHeroTalents(hero) {
  ...
  for (const id of Object.keys(state.talents)) state.talents[id] = 0;
  Object.assign(hero, state);
  return { reset: refunded > 0, refunded };
}

export function awardHeroXp(hero, outcome = {}) {
  ...
  if (!Number.isSafeInteger(waveNumber) || waveNumber < 1 || !Number.isSafeInteger(total) || total < 1
    || !Number.isSafeInteger(kills) || kills < 0 || typeof won !== 'boolean') return noReward;
