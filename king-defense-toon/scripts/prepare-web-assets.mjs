...
for (const [type, config] of Object.entries(units)) {
  const source = await loadSource(config.file);
...
const manifest = `// Generated by scripts/prepare-web-assets.mjs. Original source assets are retained.\n`
  ...
  + `]);\n`;
await writeFile(new URL('asset-web.ts', root), manifest);
await writeFile(new URL('asset-web.mjs', root), `// Temporary bridge for JavaScript callers during the staged TypeScript migration.\nexport * from './asset-web.ts';\n`);
