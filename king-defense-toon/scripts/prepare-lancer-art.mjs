...
await writeFile(new URL('../lancer-art.mjs', import.meta.url), `// Native Pixel Frog Lancer frames. Recreate with scripts/prepare-lancer-art.mjs.\nexport const LANCER_ASSETS = Object.freeze({\n${colors.map((color, index) => `  ${index + 1}: { sheet: new URL('./assets/lancer/lancer-${color.toLowerCase()}.webp', import.meta.url).href, art: new URL('./assets/lancer/lancer-${color.toLowerCase()}-art.webp', import.meta.url).href },`).join('\n')}\n});\nexport const LANCER_GEOMETRY = ${JSON.stringify(metadata, null, 2)};\n`);
