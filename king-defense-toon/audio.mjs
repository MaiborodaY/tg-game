...
function settleContextCall(context, method) {
  try {
...
export function createBattleAudio() {
  ...
  function releaseVoice(source, stopSource) {
    source.onended = null;
    if (stopSource) {
  ...
  function suspendUnwantedContext(candidate) {
  ...
  function playBowShot() {
    ...
    try {
      ...
      source.onended = () => releaseVoice(source, false);
      voices.add(source);
      source.start(now);
      lastShotAt = now;
      return true;
