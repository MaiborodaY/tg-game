import { HERO_NAME, HERO_MAX_LEVEL, HERO_BRANCHES, HERO_TALENTS, getHeroProgress, getHeroStats,
  getHeroTalentStatus, spendHeroTalent, resetHeroTalents } from './hero.ts';
...
const ICON_PATHS: Readonly<Partial<Record<string, string>>> = {
...
const icon = (name: string) => `<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${ICON_PATHS[name] ?? ICON_PATHS.light}</svg>`;
...
export function createHeroUI({ button, panel, getHero, getBattle = () => null, onChange = () => {}, close = () => {} }: HeroUIOptions): HeroUI {
  let selectedId: TalentId | null = null, lastSignature = '';
  ...
  type TextRef = 'level' | 'points' | 'xp' | 'stats' | 'timing' | 'detail-name' | 'detail-rank'
    | 'detail-description' | 'detail-gate' | 'reset-hint';
  // All references are created by the markup above, including its two action buttons.
  ...
  function statusText(talent: HeroTalentDefinition, status: HeroTalentStatus) {
  ...
  function render() {
    ...
    for (const node of nodes) {
      ...
      node.querySelector<HTMLElement>('.hero-node-gate')!.textContent = status.available ? '+' : status.reason === 'level' ? `Lv.${talent.level}` : '';
    }
  ...
  function onClick(event: MouseEvent) {
    // Native click targets here are elements inside the markup owned by this panel.
